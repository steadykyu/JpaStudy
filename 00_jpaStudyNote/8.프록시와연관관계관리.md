## 요약
+ 즉시로딩과 지연로딩을 활용하여 한 엔티티와 자주 사용되는 엔티티를 함께 나오도록 쿼리를 출력하거나, 사용시에만 쿼리를 출력하도록 할수 있다.
+ 이 과정을 이해하기 위해서는 프록시라는 개념을 알아야한다.
...
+ 참조하는 곳이 하나일 때 사용해야함
+ 즉 특정 엔티티가 개인을 소유할 때 사용
```
- Child가 Parent에만 관계가 있는 단일 소유자 관계
