# 요약
1. 객체와 DB 사이에도 객체지향적 설계가 필요하다. 
2. 이를 위한 단방향 연관관계와 양방향 연관관계가 존재한다.
3. 실제 설계는 단방향 매핑만으로 구성하는 것이 좋다.
4. 추가로 필요한 부분에 양방향 매핑을 하는 것이 좋다.
5. 양방향 매핑에는 여러가지 규칙들이 존재하며, 특히 외래키를 기준으로 연관관계 주인을 정해주어야한다. N의 관계에 주인을 설정해주면 된다.
6. 양방향 매핑때, 값이 추가되었을때는 (주인쪽, 주인이 아닌쪽) 참조객체 모두에게 값을 넣어주어야 문제가 발생하지 않는다. 이때 편의메소드를 사용하면 여러 문제상황들을 막을 수 있다.
...
+ 이 경우 select 쿼리가 날라오지않고, team은 매핑상태가 아닌 순수한 객체상태처럼 1차캐시에 존재한다.
+ 그래서 list를 사용해도 컬렉션에 값이 존재하지 않는다. 

### 순수한 객체 관계를 고려하여 양쪽다 값을 입력하는 이유
1. 위의 사고를 막고 조금더 객체지향적으로 생각한다면 양쪽에 다 값을 입력해주는 것이 좋다.
2. 테스트 케이스 작성시에는 JPA 없이도 java코드가 돌아가도록 해야하는데, JPA가 동작하지 않기때문에 객체 참조에러로 테스트에러가 발생한다.
...
+ 연관관계의 주인은 **외래 키의 위치**를 기준으로 정해야함
    - 운영관계에서도 좋고, DB설계도 쉽고, 성능도 더 좋다. 
    - 굳이 주인이 아닌쪽에서 값을 넣어야할때는 연관관계 편의메서드를 사용하면 된다.
