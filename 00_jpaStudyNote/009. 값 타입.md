## 요약
+ 값타입에는 크게는 엔티티 타입, 값타입으로 나뉜다.
+ 값 타입에는 기본값 타입, 임베디드 타입, 컬렉션타입으로 나뉜다.
+ 임베디드 타입과 컬렉션 타입은 @Embeddalbe ,@ElementCollection 으로 생성할수 있으며, 운영적으로 한눈에 보기 좋은 코드를 생성할 수 있다.
+ 둘다 좋지않은 side effect(부작용)을 가지고 있기때문에, 엔티티를 사용할때인지 값타입을 사용해야할 때인지 유의해서 사용해야한다.
+ 주로 식별자가 필요하고, 지속해서 값을 추적(조회), 변경해야 한다면 엔티티를 사용하고/ 값만 보관상태로만 두고 활용할때는 임베디드나 컬렉션 타입을 사용한다.
# 값 타입
...
```
+ remove()는 equals()와 hashcode()가 꼭 알맞게 재정의 되어있어야 한다. 만약 제대로 안되있다면 remove가 제대로 동작하지 않는다.
> 결과 쿼리
```sql
Hibernate: 
...
```
+ add()해줄때만 update문 해주는 모습을 볼 수 있다. remove된 부분(old1)은 쿼리가 생성되지 않음.
