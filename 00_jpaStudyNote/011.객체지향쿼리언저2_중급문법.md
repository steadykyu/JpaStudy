...
member1Age = 0
```
+ executeUpdate()는 flush()를 자동호출하며, 영향을 받은 엔티티 수를 int로 반환한다.
+ flush()는 db에 update 쿼리를 날려 DB MEMBER TABLE의 age 값이 20으로 변경시킨다.
+ DB의 age는 20이지만 영속성컨텍스트의 age는 값을 지정해준 적이 없으므로, int 초기값 0이 나온다.
